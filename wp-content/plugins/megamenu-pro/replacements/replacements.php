...
class Mega_Menu_Replacements {
	...
	private function do_logo_replacement( $item, $item_output ) {
		...
		$replacement = "<a class='mega-menu-link mega-menu-logo' href='" . apply_filters("megamenu_logo_url", get_home_url(), $item ) . "'><img class='mega-menu-logo' width='{$width}px' height='{$height}px' src='{$url}' /></a>";
	...
	public function add_replacements_tab( $tabs, $menu_item_id, $menu_id, $menu_item_depth, $menu_item_meta ) {
		...
		$html .= "                <a id='mm_choose_logo' style='{$choose_style}' data-menu-item-id='" . esc_attr( $menu_item_id ) . "' data-button-text='" . __("Select Icon", "megamenupro") . "'>" . __("Select Logo", "megamenupro") . "</a>";
		$html .= "                <a id='mm_remove_logo' style='{$remove_style}' data-menu-item-id='" . esc_attr( $menu_item_id ) . "'>" . __("Remove Logo", "megamenupro") . "</a>";
		$html .= "                <a id='mm_change_logo' style='{$change_style}' data-menu-item-id='" . esc_attr( $menu_item_id ) . "' data-button-text='" . __("Change Icon", "megamenupro") . "'>" . __("Change Logo", "megamenupro") . "</a>";
		$html .= "            </td>";
		$html .= "        </tr>";
		$html .= "        <tr class='logo' style='display: {$logo_display}'>";
		$html .= "            <td class='mega-name'>";
		...
		$html .= "                    <option value='expand_to_left' " . selected( $search_type, 'expand_to_left', false ) . ">" . __("Expanding", "megamenupro") . "</option>";
	...
	public function enqueue_public_scripts() {
	    ...
	    wp_enqueue_script( 'megamenu-search', plugins_url( 'js/searchbox.js' , __FILE__ ), array('jquery'), MEGAMENU_PRO_VERSION );

	    if ( function_exists('edd_get_cart_quantity') ) { // check EDD is installed & active
	    	wp_enqueue_script( 'megamenu-edd', plugins_url( 'js/edd.js' , __FILE__ ), array('jquery'), MEGAMENU_PRO_VERSION );
		}
