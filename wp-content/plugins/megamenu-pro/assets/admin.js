jQuery(document).ready(function($) {

	/** Custom Icons **/

    var mm_choose_icon_frame;

    $('#mm_choose_icon, #mm_change_icon').live('click', function(event){

        event.preventDefault();

        var link = $(event.target);

        var $this = $(this);

        // Create the media frame.
        mm_choose_icon_frame = wp.media.frames.file_frame = wp.media({
            title: megamenu_pro.file_frame_title,
            library: {type: 'image'},
            button: {
                text: $this.attr('data-button-text') // button text
            }
        });

        // When an image is selected, run a callback.
        mm_choose_icon_frame.on('select', function() {

            var selection = mm_choose_icon_frame.state().get('selection');

            selection.map( function(attachment) {
                attachment = attachment.toJSON();
                attachment_id = attachment.id;

                if (attachment.sizes.thumbnail) {
                    attachment_url = attachment.sizes.thumbnail.url;
                } else {
                    attachment_url = attachment.sizes.full.url;
                }
            });


            $("#mm_icon").attr('src', attachment_url).show();
            $("input#custom_icon_id").val(attachment_id);

            if (link.attr('id') != 'mm_change_icon') {
                $("#mm_choose_icon").toggle();
                $("#mm_change_icon").toggle();
                $("#mm_remove_icon").toggle();
            }
        });

        mm_choose_icon_frame.open();

    });

    $('#mm_remove_icon').live('click', function(e){
        e.preventDefault();

        jQuery("#mm_icon").attr('src', "").hide();
        jQuery("input#custom_icon_id").val('false');
        jQuery("#mm_choose_icon").toggle();
        jQuery("#mm_change_icon").toggle();
        jQuery("#mm_remove_icon").toggle();
    });

    /** Replacements **/

    var mm_choose_logo_frame;

    $('#mm_choose_logo, #mm_change_logo').live('click', function(event){

        event.preventDefault();

        var link = $(event.target);

        var $this = $(this);

        // Create the media frame.
        mm_choose_logo_frame = wp.media.frames.file_frame = wp.media({
            title: megamenu_pro.file_frame_title,
            library: {type: 'image'},
            button: {
                text: $this.attr('data-button-text') // button text
            }
        });

        // When an image is selected, run a callback.
        mm_choose_logo_frame.on('select', function() {

            var selection = mm_choose_logo_frame.state().get('selection');

            selection.map( function(attachment) {
                attachment = attachment.toJSON();
                attachment_id = attachment.id;

                if (attachment.sizes.thumbnail) {
                    attachment_url = attachment.sizes.thumbnail.url;
                } else {
                    attachment_url = attachment.sizes.full.url;
                }

            });


            $("#mm_logo").attr('src', attachment_url).show();
            $("input#custom_logo_id").val(attachment_id);

            if (link.attr('id') != 'mm_change_logo') {
                $("#mm_choose_logo").toggle();
                $("#mm_change_logo").toggle();
                $("#mm_remove_logo").toggle();
            }
        });

        mm_choose_logo_frame.open();

    });

    $(document).on('megamenu_content_loaded', function() {
        if ($('#codemirror').length) {

            var codeMirror = CodeMirror.fromTextArea(document.getElementById('codemirror'), {
                tabMode: 'indent',
                lineNumbers: true,
                lineWrapping: true
            });

            codeMirror.on("change", function(cm, change) {
                $('#codemirror').html(cm.getValue());
            });

        }
    });


    // Refresh CodeMirror
    $('.replacements').live('click', function() {
        $('.CodeMirror').each(function(i, el){
            el.CodeMirror.refresh();
        });
    });

    $('#mega_replacement_type').live('change', function() {
        $('.CodeMirror').each(function(i, el){
           setTimeout(function(){
              el.CodeMirror.refresh();
            }, 50);
        });
    });

    $('#mm_remove_logo').live('click', function(e){
        e.preventDefault();

        jQuery("#mm_logo").attr('src', "").hide();
        jQuery("input#custom_logo_id").val('false');
        jQuery("#mm_choose_logo").toggle();
        jQuery("#mm_change_logo").toggle();
        jQuery("#mm_remove_logo").toggle();
    });

    $('select#mega_replacement_mode').live("change", function() {
        var select = $(this);
        var selected = $(this).val();
        select.next().children().hide();
        select.next().children('.' + selected).show();
    });

    $('select#mega_replacement_type').live("change", function() {
        var select = $(this);
        var selected = $(this).val();
        $(".replacements table tr").not(".type").hide();
        $(".replacements table tr." + selected).show();
    });

    $('.mm_tab.replacements').live('click', function() {

        $(".mm_colorpicker").spectrum({
            preferredFormat: "rgb",
            showInput: true,
            showAlpha: true,
            clickoutFiresChange: true,
            change: function(color) {
                if (color.getAlpha() === 0) {
                    $(this).siblings('div.chosen-color').html('transparent');
                } else {
                    $(this).siblings('div.chosen-color').html(color.toRgbString());
                }
            }
        });

    });

	/** Roles **/

    $('#mm_roles select').live('change', function() {

        var option = $(this);

        if (option.val() == 'by_role') {
            $('#mm_roles input[type=checkbox]').removeAttr('disabled');
        } else {
            $('#mm_roles input[type=checkbox]').attr('disabled', 'disabled');
        }

    });

    /** Style Overrides **/

    $('.override_toggle_enabled').live('change', function() {
        var checkbox = $(this);
        var checked = checkbox.is(":checked");

        var inputs = checkbox.parent().siblings().find('input, select');

        inputs.each(function() {

            var name = $(this).attr('name');

            if (checked) {
                name = name.replace('disabled', 'enabled');
            } else {
                name = name.replace('enabled', 'disabled');
            }

            $(this).attr('name', name);

        });

        var parent = checkbox.parent().parent();

        parent.toggleClass('mega-enabled', 'mega-disabled');

    });

    $('.mm_tab.styling').live('click', function() {

        $(".mm_colorpicker").spectrum({
            preferredFormat: "rgb",
            showInput: true,
            showAlpha: true,
            clickoutFiresChange: true,
            change: function(color) {
                if (color.getAlpha() === 0) {
                    $(this).siblings('div.chosen-color').html('transparent');
                } else {
                    $(this).siblings('div.chosen-color').html(color.toRgbString());
                }
            }
        });

    });


    var mm_choose_background_frame;

    $('#mm_choose_background, #mm_change_background').live('click', function(event){

        event.preventDefault();

        var link = $(event.currentTarget);

        var $this = $(this);

        // Create the media frame.
        mm_choose_background_frame = wp.media.frames.file_frame = wp.media({
            title: megamenu_pro.file_frame_title_background,
            library: {type: 'image'},
            button: {
                text: $this.attr('data-button-text') // button text
            }
        });

        // When an image is selected, run a callback.
        mm_choose_background_frame.on('select', function() {

            var selection = mm_choose_background_frame.state().get('selection');

            selection.map( function(attachment) {
                attachment = attachment.toJSON();
                attachment_id = attachment.id;

                if (attachment.sizes.thumbnail) {
                    attachment_url = attachment.sizes.thumbnail.url;
                } else {
                    attachment_url = attachment.sizes.full.url;
                }

            });


            $("#mm_background").attr('src', attachment_url).show();
            $("input#custom_background_id").val(attachment_id);

            if (link.attr('id') != 'mm_change_background') {
                $("#mm_choose_background").toggle();
                $("#mm_change_background").toggle();
                $("#mm_remove_background").toggle();
            }
        });

        mm_choose_background_frame.open();

    });

    $('#mm_remove_background').live('click', function(e){
        e.preventDefault();

        jQuery("#mm_background").attr('src', "").hide();
        jQuery("input#custom_background_id").val('false');
        jQuery("#mm_choose_background").toggle();
        jQuery("#mm_change_background").toggle();
        jQuery("#mm_remove_background").toggle();
    });

});