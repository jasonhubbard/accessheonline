...
<?php if ( $show_content ) : ?>
	...
	<?php
    /**
     * Use Lesson Topics repeat to show first instruction
     */
    ?>
	<?php if ( ! empty( $topics ) ) : $count=0;
			 foreach ( $topics as $key => $topic ) : $count++; ?>
                        <?php $completed_class = empty( $topic->completed ) ? 'topic-notcompleted' : 'topic-completed';
						if($count==1){
							$firstmodule=$topic->post_title;
						}
						 ?>
                    <?php endforeach; ?>
   ...
   <?php if($completed_class=="topic-notcompleted"): ?>
    <p class="module-instruction">Begin by clicking on '<?php echo $firstmodule; ?>' below.</p>
    <?php endif; ?>
	...
	if ( $post->post_type == 'sfwd-lessons' ) {
		$progress = learndash_get_course_progress( null, $post->ID );
			...
			// show instruction if topics are completed but lesson hasn't been confirmed completed
			if ( (empty( $progress['this']->completed )) && (learndash_lesson_topics_completed( $post->ID )) ) { ?>
				<p class="module-instruction">You have now completed all the above modules for '<?php echo the_title(); ?>'. We hope you enjoyed the session.</p>
                <p class="module-instruction">Please now click on the ‘Mark complete’ button below to finish.</p>
			<?php }
		}
		?>
    
    <?php if($completed_class=="topic-completed"): ?>
...
<?php /*<p id="learndash_next_prev_link"><?php echo learndash_previous_post_link(); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo learndash_next_post_link(); ?></p>*/ ?>